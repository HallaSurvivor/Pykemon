...
class Attack(object):
    def __init__(self,
                 ...
                 cause_player_switch=False, sound_based=False, ohko=False,
            ...
            if mod > 0:
                if target.stages[i] == 6:
                    p.add_to_print_buffer(
                        "{name}'s {stat} won't go any higher!".format(name=target.name, stat=r.stat_list[i]),
                        required_pokemon=[user, target])

                elif mod == 1:
                    p.add_to_print_buffer("{name}'s {stat} rose!".format(name=target.name, stat=r.stat_list[i]),
                                          required_pokemon=[user, target], style=r.Style.modify, target=target,
                                          modifier=1, modified_stat=i)

                elif mod == 2:
                    p.add_to_print_buffer("{name}'s {stat} sharply rose!".format(name=target.name, stat=r.stat_list[i]),
                                          required_pokemon=[user, target], style=r.Style.modify, target=target,
                                          modifier=2, modified_stat=i)

                elif mod == 3:
                    p.add_to_print_buffer(
                        "{name}'s {stat} drastically rose!".format(name=target.name, stat=r.stat_list[i]),
                        required_pokemon=[user, target], style=r.Style.modify, target=target, modifier=3,
                        modified_stat=i)

            elif mod < 0:
                if target.stages[i] == -6:
                    p.add_to_print_buffer(
                        "{name}'s {stat} won't go any lower!".format(name=target.name, stat=r.stat_list[i]),
                        required_pokemon=[user, target])

                elif mod == -1:
                    p.add_to_print_buffer("{name}'s {stat} fell!".format(name=target.name, stat=r.stat_list[i]),
                                          required_pokemon=[user, target], style=r.Style.modify, target=target,
                                          modifier=-1, modified_stat=i)

                elif mod == -2:
                    p.add_to_print_buffer("{name}'s {stat} harshly fell!".format(name=target.name, stat=r.stat_list[i]),
                                          required_pokemon=[user, target], style=r.Style.modify, target=target,
                                          modifier=-2, modified_stat=i)

                elif mod == -3:
                    p.add_to_print_buffer(
                        "{name}'s {stat} drastically fell!".format(name=target.name, stat=r.stat_list[i]),
                        required_pokemon=[user, target], style=r.Style.modify, target=target, modifier=-3,
                        modified_stat=i)
        ...
        for i in range(len(r.nonvolatile)):
            if self.status == r.nonvolatile[i]:
                ...
                if target.status_nonvolatile == "healthy":
                    p.add_to_print_buffer("{pokemon} was {status}".format(pokemon=target.name, status=self.status),
                                          required_pokemon=[user, target], style=r.Style.nonvolatile, target=target,
                                          status=self.status)
                else:
                    p.add_to_print_buffer(
                        "{pokemon} was already {status}".format(pokemon=target.name, status=target.status_nonvolatile),
                        required_pokemon=[user, target])
            ...
            if self.status == r.volatile[i]:
                if target.volatile[self.status] == False:
                    ...
                    if self.status != "flinch":
                        p.add_to_print_buffer("{pokemon} was {status}".format(pokemon=target.name, status=self.status),
                                              required_pokemon=[user, target], target=target, style=r.Style.status,
                                              status=self.status)
                else:
                    p.add_to_print_buffer(
                        "{pokemon} was already {status}".format(pokemon=target.name, status=self.status),
                        required_pokemon=[user, target])
    ...
    def use(self, user, target):
        ...
        while use_state != "end":
            ...
            if use_state == "set last move":
            ...
            elif use_state == "check flinch":
                if user.volatile["flinch"] is True:
            ...
            elif use_state == "check confusion":
                if user.volatile["confused"] is False:
                ...
                p.add_to_print_buffer("{pokemon} hurt itself in its confusion!".format(pokemon=user.name),
                                      style=r.Style.damage, target=user, damage=damage, required_pokemon=[user])
                use_state = "end"
                ...
                if user.status_nonvolatile == "healthy":
                    ...
                    if user.status_nonvolatile == "frozen":
                        ...
                        if user.status_counter == 1:
                            p.add_to_print_buffer(
                                "{pokemon} was paralyzed and unable to move!".format(pokemon=user.name),
                                required_pokemon=[user])
                            use_state = "end"
                        else:
                            use_state = "use move"
                ...
                if self.payday is True:
                    r.payday_count += 1
                use_state = "check ohko"

            elif use_state == "check ohko":
                if self.ohko is True:
                    acc = 30 + (user.level - target.level)

                    if randint(1, 100) <= acc:
                        p.add_to_print_buffer("It's a one-hit KO!", required_pokemon=[user], damage=target.hp,
                                              style=r.Style.damage, target=target)
                        use_state = "end"
                ...
                if recoil_damage != 0:
                    p.add_to_print_buffer("{pokemon} was hurt by recoil!".format(pokemon=user.name),
                                          style=r.Style.damage, target=user, damage=recoil_damage,
                                          required_pokemon=[user])
                use_state = "check regain health"

            elif use_state == "check regain health":
                if self.regain_health is True:
                    regained_health = int(float(damage) / 2)
                    p.add_to_print_buffer("{pokemon} had its energy drained!".format(pokemon=target.name),
                                          style=r.Style.damage, target=user, damage=-regained_health,
                                          required_pokemon=[user])
                use_state = "modify status"
